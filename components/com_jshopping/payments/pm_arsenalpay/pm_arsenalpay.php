...
/////////////////////////////////////////////////////////////
	...
	function checkTransaction($pmconfigs, $order, $act)
	{
		...
		foreach( $keyArray as $key ) {
			if( !array_key_exists( $key, $ars_callback ) ){
				echo 'ERR_'. $key;
		...
		$lessPayment = false;
		$totalAmount = number_format( $order->order_total / $order->currency_exchange, 2, '.', '' );
		if ( $ars_callback['MERCH_TYPE'] == '0' && $totalAmount == $ars_callback['AMOUNT'] ) {
			$lessPayment = false;
		}
		elseif ( $ars_callback['MERCH_TYPE'] == '1' && $totalAmount >= $ars_callback['AMOUNT'] && $totalAmount == $ars_callback['AMOUNT_FULL'] ) {
			$lessPayment = true;
		}
		else {
			echo 'ERR_AMOUNT';
		...
		if ( ($ars_callback['FUNCTION'] == "payment") && ($ars_callback['STATUS'] == "payment") ) {
			...
			echo 'OK';
			if ( $lessPayment ) {
				return array(1, 'OK. Order ID ' . $order->order_id . '. Payed amount is ' . $ars_callback['AMOUNT']);
			}
			else {
				return array(1, 'OK. Order ID ' . $order->order_id . '. Payed full amount.');
			}
		}
		else {
			echo 'ERR_FUNCTION';
